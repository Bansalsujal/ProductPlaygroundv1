...
app.put('/api/user/update', async (req, res) => {
  try {
    ...
    res.json({
      ...
      career_stage: data.user.user_metadata?.career_stage || 'junior',
      industry: data.user.user_metadata?.industry || 'tech'
    });
  } catch (error) {
...
// Only start the server when not in serverless mode (when directly executed)
// This allows the module to be imported by serverless functions without starting a server
